...
const MyAppointments = () => {
    ...
    useEffect(() => {
        try {
            ...
            if (!user) {
                ...
                axios.get(`http://localhost:3000/appointments/user/${user.newLogin.id}`).then(response => {
    ...
    const handleOnCancel = async (id) => {
        try {
            ...
            setAppointment((renderAppoints) =>
                renderAppoints.map((appointment) => appointment.id === id ? { ...appointment, status: "cancelado" } : appointment)
            )
